...
1. JSON에 대한 설명으로 옳은 것을 모두 고르세요.

   1. JSON 형식의 데이터는 JavaScript의 객체와 완벽히 호환되므로, Javascript 객체의 다양한 메소드를 사용할 수 있다.
   2. JSON 형식의 데이터에서 속성 이름은 반드시 쌍따옴표로 둘러주어야 한다.
   3. JavaScript에서 빌트인 객체인 JSON의 `JSON.stringify()` 메소드를 이용하면 Javascript 객체를 JSON 형식으로 변환할 수 있다.
   4. `JSON.stringify()`를 이용한 직렬화 과정에서 속성 값이 `undefined`인 속성은 제외된다.
...
3. 다음 코드의 실행 결과, factorialRect(3)의 반환값은 얼마일까요?
...
5. Map의 설명으로 옮은 것을 고르세요.

   1. 속성 접근자 문법을 통해서 조작할 수 있다.
   2. length 속성을 통해서 내부에 들어있는 데이터의 갯수를 알수 있다.
   3. Map 객체 안에 들어있는 데이터는 프로토타입의 영향을 받는다.
   4. 메소드를 통해서 내부의 테이터를 조작한다.
   5. Map 객체의 키로 숫자는 사용 할 수 없다.
