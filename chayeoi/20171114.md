...
1. CSS2에서는 가운데 정렬이 없지만(margin:auto;는 트릭에 불과하다) CSS3에서는 flexbox를 이용하면 가운데 정렬이 가능하다.
...
4. `margin: auto;`는 브라우저 뷰포트에서 width를 제외한 나머지를 자동으로 절반으로 나누어 처리한다. 즉, 실제로 가운데 정렬이 일어난 것이 아니라 **margin 영역은 눈에 보이지 않는 영역이기 때문에 가운데 정렬처럼 보이는 것**이다.
