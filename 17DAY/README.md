...
1. 나이가 20세 이상이면 '성년입니다.', 20세 미만이면 '미성년입니다.'를 출력하는 adult 함수를 빈 칸을 채워서 완성하세요.
...
2. ①, ②번 코드에서 `boo()`의 실행 후, `console.log(name)`의 출력 결과는 무엇입니까? (Lexical Scope)
...
3. 'Hello World'를 출력하는 다음 함수를 실행하기 위한 코드는 무엇입니까?

   ```javascript
   const foo = function sayHello() {
...
4. 배열 `arr`의 각 요소를 제곱한 값을 요소로 갖는 새로운 배열을 만들기 위한 코드를 작성하세요. (힌트 - Array.prototype.map() 이용) 
   ...
   ```javascript
   changedarr = arr.map((item) => item ** 2)
   ```

   <br />

5. 사용자로부터 수를 입력받아 num이라는 변수에 저장하고 num이 숫자가 아니면 "숫자가 아닙니다."를, num이 숫자이면 (1부터 num까지 모두 곱한 값)을 출력하세요(예를 들어, 입력값이 6이면 "6! = 720"을 출력). (힌트 - for문과 if문을 이용하고, 사용자로부터 값을 입력받으려면 `prompt()`를 이용, 입력값을 정수로 바꾸려면 `parseInt()`를 이용, 입력값이 정수가 맞는지 확인하려면 `Number.isNaN()`을 이용)
